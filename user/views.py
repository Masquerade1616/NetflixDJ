...
# kullanıcı olusturma giriş yapma ve çıkış yapma için gerrekli importlar
from django.contrib.auth.models import User
from django.contrib.auth import authenticate, login, logout
#__________________________
from .forms import *
# Create your views here.
# mail gönderme işlemi için gereken importlar
    ...
    if request.method == 'POST':
        ...
        if password1 == password2:
            if User.objects.filter(username = username).exists():
                ...
                message = 'bu netflix projesini neos yazılım ekibiyle yaptık'
                send_mail(
                    subject,
                    message,
...
def userLogin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, 'Kullanıcı adı veya şifre hatlı')
